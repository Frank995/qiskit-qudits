"""Unitarity and encoding safety of the controlled qudit gate matrices.

The single-qudit suite checks these properties on a contiguous
leading block; a controlled gate such as ``SUM_X`` couples a target
qudit to one or more control qudits of possibly different dimension,
so the valid subspace is no longer contiguous: a basis state is valid
only when *every* constituent qudit level -- target and controls
alike -- is below its own dimension. The checks here therefore work
on explicit index sets:

* the full ``2**N x 2**N`` matrix is unitary;
* the ``prod(d_i) x prod(d_i)`` block spanned by the valid states is
  already unitary on its own, so probability never flows out of the
...
#: ``(target dim, control dims)`` register layouts.
CONTROLLED_CONFIGS = (
    (2, (2,)),
    (4, (2,)),
    (2, (4,)),
...
GATE_FACTORIES: tuple[
    tuple[str, Callable[[int, tuple[int, ...]], Gate]],
    ...,
] = (
    ("SUMX", QuditSUMXGate),
...
def _gate_cases(*, leaky_only: bool) -> list[object]:
    ...
    """
    cases: list[object] = []
    for name, make in GATE_FACTORIES:
        for target, controls in CONTROLLED_CONFIGS:
            if leaky_only and not _has_leakage((*controls, target)):
                continue
            cases.append(
                pytest.param(
                    lambda t=target, c=controls, make=make: make(t, c),
                    id=f"{name}-{_config_id(target, controls)}",
                ),
            )
    return cases
...
parametrize_gate_factories = pytest.mark.parametrize(
    "make_gate",
    [pytest.param(make, id=name) for name, make in GATE_FACTORIES],
)
...
@pytest.mark.slow
@parametrize_gate_factories
def test_matrix_is_unitary_at_the_maximum_dimension(
    make_gate: Callable[[int, tuple[int, ...]], Gate],
) -> None:
    """Check the largest supported target and control stay unitary."""
    gate = make_gate(MAX_DIM, (MAX_DIM,))
    matrix = gate_matrix(gate)
